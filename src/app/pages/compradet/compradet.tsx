...
const columns_ = [
  {
    ...
    cell: (row: any) => (
      <div>
        <Link to={"/compradetform?id=" + row._id} 
        className="btn btn-icon btn-outline ">
          <i className="fa-solid fa-edit "></i>
        </Link>
      </div>
    ), selector: (row: any) => row.id_compradet, sortable: true
  },
    { name: 'ProductoId', 
    ...
    { name: 'Compra', 
    selector: (row: any) => row.compra, sortable: true },        
];
export function CompradetPage() {
    ...
    useEffect(() => {
        compradetDataService.getcompradet()
          .then(response => response.json())
          .then(response => {
            setcompradet(response)
            ...
            <div className="col-lg-12">
              <div className="card card-custom">
                <div className="card-header bg-dark ">
                  <h3 className="card-title  text-light">Listado de Compradet</h3>
                  <div className="card-toolbar">
                    <Link to={"/compradetform"} className="btn btn-sm btn-primary ">
                      <i className="fa-solid fa-file fs-1x text-light"></i>
                      Nuevo Compradet
                    </Link>
                    <button  className="btn btn-success btn-sm"
                    onClick={(e) => exportToCSV(compradet,'Reporte de Compradet')}>
